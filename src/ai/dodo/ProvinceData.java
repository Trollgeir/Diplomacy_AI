...
public class ProvinceData {
	...
	public ProvinceData(Power power, Province province, ArrayList<Power> powers) {
		...
		computeAdjProvinces();
		System.out.println("Province: " + province.daide() + "with adjecent provinces:");
		for (Province p : adjProvinces) {
			System.out.println("\t" + p.daide());
		}
	...
	public void update(ArrayList<Unit> usedUnits) {
		...
		for (Node node : nodes) {
			if (node.unit != null) {
				...
				if (!containsUnit(unit)) {
					...
					Node destNode = findDestNode(node, province);
					if (destNode != null) {
	...
	private Node findDestNode(Node src, Province destProv) {	
		//add to destNodes all the nodes that are reachable from src 		
		ArrayList<Node> destNodes = getAdjNodes(src);
		for (Node dest : destNodes) {
			if (dest.province == destProv) return dest;
		}

		System.out.println("No node found at findDestNode!!!");
		return null;
	}
	...
	private ArrayList<Node> getAdjNodes(Node node) {
		ArrayList<Node> nodes = new ArrayList<Node>();
		nodes.addAll(node.allNeighbors());
		nodes.add(node); 
		ArrayList<Node> list = new ArrayList<Node>();
		
		for (Node n : nodes) {

			System.out.println(n.daide().toString());


			if (node.isLand()) {
				if (n.isLand()) list.add(n);
			} else if (node.isSea()) {
				if (n.isSea() || n.isCoast()) list.add(n);
			} else if (node.isCoast()) {
				if (n.isCoast() || n.isSea()) list.add(n);
			} else {
				System.out.println("none of them?");
				if (!(node.isLand()|| node.isSea()|| node.isCoast())) System.out.println("noooooooooooooooooooooooooooooo");
			}
	...
	private void computeGains(Power power) {
		...
		gains = c_supply * getSupplyWeight(province);
		
		for (Province p : adjProvinces) {
			//gains += c_adjSupply  * getSupplyWeight(p) * isOwnCenter(province, power) * defence; 
			gains += c_adjSupply  * getSupplyWeight(p);
		}
	}
	...
	public ArrayList<Unit> getAvailebleUnits() {
		int powerIdx = powers.indexOf(power);
